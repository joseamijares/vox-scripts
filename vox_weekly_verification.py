...
# Load secrets from 1Password first, then env, then .env
sys.path.insert(0, str(Path.home() / ".hermes" / "scripts"))
...
class VoxWeeklyVerification:
    ...
    def check_2_contradictions(self):
        ...
        # Filter out false positives (grade=0 vs grade=0)
        real_contra = [c for c in contra_list if c['vox_grade'] > 0 or c['unified_grade'] > 0]
        self.stats['contradictions'] = len(real_contra)
        if real_contra:
            self.issues.extend([f"{c['ticker']}: VOX {c['vox_action']}({c['vox_grade']}) vs Unified {c['unified_action']}({c['unified_grade']})" for c in real_contra[:5]])
            print(f"  🔴 {len(real_contra)} contradictions")
        else:
    ...
    def check_7_broker_sync(self):
        ...
        # Use 'shares' not 'units' - column was renamed
    ...
    def run(self):
        ...
        # Exit with non-zero if critical issues — but only for REAL issues
        # (exclude grade=0 vs grade=0 comparisons as false positives)
        real_issues = [i for i in self.issues if 'Grade 0 vs 0' not in str(i) and 'grade=0' not in str(i).lower()]
        if real_issues:
            print(f"\n🚨 {len(real_issues)} CRITICAL ISSUES — NOTIFICATION TRIGGERED")
            return 1
        return 0
