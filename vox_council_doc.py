...
def fetch_all_sectors(cur, sectors: set) -> Dict[str, Dict]:
    ...
    cur.execute("""
        ...
        WHERE table_name IN ('sp500_sector_leaders', 'sp500_sector_momentum', 'sector_momentum')
    """)
    available_tables = [r[0] for r in cur.fetchall()]
    
    if not available_tables:
        # No sector tables available - return empty
        return {}
    ...
    try:
        if table_name == 'sp500_sector_leaders':
            # Check if rank column exists
            cur.execute("""
                SELECT column_name FROM information_schema.columns 
                WHERE table_name = 'sp500_sector_leaders' AND column_name = 'rank'
            """)
            has_rank = cur.fetchone() is not None
            ...
            if has_rank:
                ...
                # No rank column - fetch all and limit in Python
                cur.execute(f"""
                    SELECT sector, ticker, momentum_score, change_5d_pct
                    FROM sp500_sector_leaders WHERE sector IN ({placeholders})
                """, tuple(sectors))
        else:
            # Use other table
            cur.execute(f"""
                SELECT sector, ticker, momentum_score, change_5d_pct
                FROM {table_name} WHERE sector IN ({placeholders})
            """, tuple(sectors))
        ...
        for row in cur.fetchall():
            ...
            if sector not in results:
                results[sector] = {"leaders": []}
            results[sector]["leaders"].append({
                "ticker": row[1], "momentum": row[2], "change_5d": row[3]
            })
        return results
