#!/usr/bin/env python3
"""
VOX Smart Alert System v8 — LLM-Enhanced Intelligence

Flow:
1. Detect raw signals (stop, move, news, trump)
2. Pass through LLM layer for context + reasoning
3. LLM decides: should_alert (yes/no), priority, exact action
4. Only send if LLM approves
5. Rich, personalized messages with portfolio context

Rules:
- LLM is the final gatekeeper — no alert without approval
- Every alert explains WHY it matters to YOUR portfolio
- Exact actions with numbers (shares, prices, % of portfolio)
...
def call_llm(prompt: str, max_tokens: int = 800) -> str:
    ...
    data = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are VOX, an elite trading intelligence system managing a $203K portfolio. Be razor-sharp concise. Every word must earn its place. Use exact numbers. No fluff."
            },
...
def llm_enhance_stop(ticker: str, stop_price: float, current_price: float, 
    ...
    """LLM decides if stop alert is worth sending."""
...
VALUE: ${position.get('live_value', 0):,.0f} ({position.get('live_value', 0)/portfolio['total_value']*100:.1f}% of portfolio)
UNREALIZED: {((current_price - position.get('avg_cost', 0))/position.get('avg_cost', 1)*100):+.1f}%

MACRO: {macro['regime']} | VIX {macro['vix']:.1f}

Should this alert be sent? Respond ONLY with JSON:
{{"send": true/false, "priority": "CRITICAL", "headline": "one punchy line", "action": "exact action with numbers", "why": "one sentence on portfolio impact"}}"""
    
    response = call_llm(prompt, 400)
...
def llm_enhance_move(ticker: str, change_pct: float, position: Dict, 
    ...
    """LLM decides if move alert is worth sending."""
    
    prompt = f"""BIG MOVE — {ticker} {change_pct:+.1f}%

POSITION: ${position.get('live_value', 0):,.0f} ({position.get('live_value', 0)/portfolio['total_value']*100:.1f}% of portfolio)
AVG COST: ${position.get('avg_cost', 0):.2f} | CURRENT: ${position.get('live_price', 0):.2f}
UNREALIZED: {((position.get('live_price', 0) - position.get('avg_cost', 0))/position.get('avg_cost', 1)*100):+.1f}%

MACRO: {macro['regime']} | SECTOR ROTATION: {macro['sector_rotation']}

Is this move significant enough to alert? Respond ONLY with JSON:
{{"send": true/false, "priority": "HIGH/MEDIUM", "headline": "one punchy line", "action": "exact action", "why": "portfolio context"}}"""
    
    response = call_llm(prompt, 500)
...
def llm_enhance_news(ticker: str, headline: str, relevance: int, 
    ...
    """LLM decides if news alert is worth sending."""
...
{{"send": true/false, "priority": "HIGH/MEDIUM", "headline": "one punchy line", "action": "exact action", "why": "why it matters to this portfolio"}}"""
...
Write a tight, insightful digest. 3-4 sentences max. Include:
1. Key takeaway from today
2. One position to watch tomorrow
3. Macro implication

No generic advice. Use actual numbers."""
    
    response = call_llm(prompt, 600)
...
def generate_alerts():
    ...
    stop_alerts = check_stops(state, positions, portfolio, macro)
    all_alerts.extend(stop_alerts)
    
    move_alerts = check_moves(state, positions, portfolio, macro)
    all_alerts.extend(move_alerts)
    
    news_alerts = check_news(state, positions, portfolio)
    all_alerts.extend(news_alerts)
    
    digest_alerts = check_digest(state, positions, macro)
